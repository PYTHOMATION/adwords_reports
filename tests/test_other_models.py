...
def test_close_variant():
    ...
    assert TextHandler("Äm-berg', Oberpfalz").standardized_city() == "Äm berg'"
    assert TextHandler("Äm-be'rg, Oberpfalz").standardized_city(decode=True) == "Am berg"
    assert TextHandler("Hagen (Westfalen)").standardized_city() == "Hagen"

    assert TextHandler.remove_forbidden_adwords_chars({"asd(s)ad", "{asd(s)ad"}) == {"asdsad"}
    assert TextHandler.remove_double_space("    qweq asd  qqr") == "qweq asd qqr"
